...
var folder:         String
var stages:         Array = []  # pour stage-race
...
static func load_race(race_folder: String) -> Race:
	...
	r.stages         = data.get("stages",          [])

	# Pour une stage-race, distance_km = somme des étapes
	if r.type == "stage-race" and not r.stages.is_empty():
		var total: int = 0
		for s in r.stages:
			total += int(s.get("distance_km", 0))
		r.distance_km = total
	else:
...
func is_stage_race() -> bool:
	return type == "stage-race"

func get_stage_count() -> int:
	return stages.size()

func get_stage(n: int) -> Dictionary:
	for s in stages:
		if int(s.get("stage_number", 0)) == n:
			return s
	return {}

func get_sectors_for_stage(stage_number: int) -> Array:
	var s := get_stage(stage_number)
	if s.is_empty():
		return []
	return s.get("profile", {}).get("sectors", [])
